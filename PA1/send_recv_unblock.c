...
int main(int argc,char *argv[])
{
    ...
    FILE *fp_rec, *fp_send;
    fp_rec = fopen("results_rec_unblock.csv", "w+");
    fp_send = fopen("results_send_unblock.csv", "w+");
    ...
    assert(p>=2);
    int tSend, tRecv, dest, i, m, flag = 0;
    char *x, *y;
    ...
    for (m = 1; m < 100000000; m = m*2){
        ...
        for (i = 0; i < 15; i++){
            if(rank==1) {
                ...
                MPI_Status status;
                gettimeofday(&t1,NULL);
            MPI_Irecv(y,m,MPI_CHAR,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD, &request);
